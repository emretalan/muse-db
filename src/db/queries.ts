...
function buildCandidateQuery(
  ...
  // Era filter. `range` null ise dönem hiç uygulanmıyor: tanınmayan bir slug
  // (`"2010s"` gibi) töreni çökertmek yerine "herhangi bir dönem" sayılıyor.
  const eraRange = filters.era && !options.skipEra ? eraToYearRange(filters.era) : null;
  if (eraRange) {
...
export async function countEraFacets(
  ...
  for (const era of ERA_KEYS) {
    // `ERA_KEYS`'ten geliyor, yani tabloda karşılığı kesin var.
 ...
 *
 * Beş boyut var (ruh hâli, yaş, bilinirlik, ve medya türüne göre yayıncı ya
 * da stüdyo) ve her biri **kendi**
 ...
 * kovaları (`net:netflix`) ve stüdyo kovaları (`std:a24`) ayrı ad
 * alanlarında.
 *
...
export async function recordPick(
  ...
  movieId: number,
  filters: PickFilters,
  /** Seçim bir tematik sezonun ritüelinden geldiyse o sezonun slug'ı. */
  seasonSlug: string | null = null
): Promise<void> {
  await pool.query(
    `INSERT INTO user_picks (session_id, movie_id, filters, season_slug, created_at)
     VALUES ($1, $2, $3, $4, NOW())`,
    [sessionId, movieId, JSON.stringify(filters), seasonSlug]
  );
}
 ...
 * sunucu hâlâ eylülde olabilir.
 */
export async function countSeasonStarters(
  seasonSlug: string,
  year: number,
  month: number
): Promise<number> {
